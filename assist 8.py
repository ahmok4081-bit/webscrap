...
#     rows.append([id, tag, text])
    """Writes accumulated CSV rows to a file with proper spreadsheet formatting.

    Filters out empty or whitespace-only rows before writing to ensure data
    integrity.

    Args:
        fileName: The output destination file path.
    """
...
def extract_tags_from_chapter(version_id: int, chapter_tag: Tag, rows: list, lineNumber: int, file_name: str) -> list[model.Entry]: 
    """Parses a chapter's BeautifulSoup HTML structure into stylized entries.

    Traverses child, child of child, and nephew tags, maps typographical styles
    to sequential structures, handles footnotes, and accumulates plain-text verses.

    Args:
        version_id: Translation ID (e.g., 107 for NET).
        chapter_tag: The BeautifulSoup Tag containing chapter HTML.
        rows: Global accumulation list for CSV export logs.
        lineNumber: Current sequential row ID sequence.
        file_name: Output filename prefix (e.g., "NET_LUK_015").

    Returns:
        A list of parsed Entry dataclass objects.
    """
                ...
                if isinstance(child, Tag):
                    ...
                    for nephew in child.children:
                        if isinstance(nephew, Tag) and nephew.get_text(strip=True): # check if nephew is a Tag and not empty or just whitespace
                            ...
                            if {'note'}.issubset(classes):
                                # if NET bible parse the footnote content to split the footnote content into multiple lines based on the footnote markers (tn, sn, tc, map)                       
                                if version_id==107:
                                ...
                                if {'verse', 'label'}.issubset(classes):
                                    lineNumber = append_verse_plain_text(entries, rows, lineNumber, verse_qtt, file_name)
                                    verse_qtt += 1
                            ...
                            if {'s1', 'heading'}.issubset(classes):
                                lineNumber = append_verse_plain_text(entries, rows, lineNumber, verse_qtt, file_name)
...
def get_chapter_data(version: str, book_id: int, reference: str) -> dict:
    """Fetches chapter layout HTML and metadata from the bible.com API.

    Args:
        version: API version string (e.g., "3.3").
        book_id: Version/Translation numeric ID (e.g., 107 for NET).
        reference: Standard USFM chapter selector (e.g., "LUK.15").

    Returns:
        A dictionary containing deserialized API JSON response.

    Raises:
        requests.exceptions.HTTPError: If the remote HTTP request fails.
    """
...
def load_ot_books_from_json(path: str) -> List:
    """Loads Old Testament book configurations from a flat JSON manifest.

    Args:
        path: Path to the directory containing old.json.

    Returns:
        A list of deserialized book configuration dictionaries.
    """
    ot = {}
    nt = {}  
...
def load_nt_books_from_json(path: str) -> List:
    """Loads New Testament book configurations from a flat JSON manifest.

    Args:
        path: Path to the directory containing new.json.
...
def get_translation_by_id(
    ...
    """Fetches a translation document from MongoDB by its abbreviation.

    Args:
        translation_id: Target translation abbreviation (e.g., "ESV").
        db_name: MongoDB database name. Defaults to "bible".
        collection_name: MongoDB collection name. Defaults to "translation".

    Returns:
        The matched translation dictionary (BSON), or None if not found.
    """
...
def get_book_from_translation(translation: dict | None, book_id: str) -> dict | None:
    """Retrieves a book dictionary from a translation record by ID or abbreviation.

    Args:
        translation: The parent translation document (dictionary), or None.
        book_id: 3-letter USFM book code (e.g., "LUK").

    Returns:
        The matched book dictionary, or None if not found or translation is None.
    """
    if not translation:
        return None
    return next(
...
def get_chapter_from_book(book: dict | None, chapter_number: int) -> dict | None:
    """Retrieves a chapter dictionary from a book dictionary by chapter index.

    Args:
        book: The parent book document (dictionary), or None.
        chapter_number: Chapter sequence index (1-based).

    Returns:
        The matched chapter dictionary, or None if not found or book is None.
    """
    if not book:
        return None
    return next(
...
def dict_to_entry(entry: dict) -> model.Entry:
    """Deserializes a raw dictionary representation into an Entry dataclass.

    Args:
        entry: A dictionary containing entry fields.

    Returns:
        A strongly-typed Entry instance.
    """
    return model.Entry(
...
def dict_to_chapter(chapter: dict) -> model.ChapterDoc:
    """Deserializes a raw dictionary representation into a ChapterDoc dataclass.

    Args:
        chapter: A dictionary containing chapter fields.

    Returns:
        A strongly-typed ChapterDoc instance.
    """
...
def dict_to_book(book: dict) -> model.BookDoc:
    """Deserializes a raw dictionary representation into a BookDoc dataclass.

    Args:
        book: A dictionary containing book fields.

    Returns:
        A strongly-typed BookDoc instance.
    """
...
def dict_to_translation(translation: dict) -> model.TranslationDoc:
    """Deserializes a raw dictionary representation into a TranslationDoc dataclass.

    Args:
        translation: A dictionary containing translation fields.

    Returns:
        A strongly-typed TranslationDoc instance.
    """
...
def find_book_in_translation_doc(
    ...
    """Finds a BookDoc within a TranslationDoc by book ID or abbreviation.

    Args:
        translation: The parent TranslationDoc object.
        book_id: 3-letter USFM book code or abbreviation.

    Returns:
        The matched BookDoc object, or None if not found.
    """
    return next(
        (
            book
...
def find_chapter_in_book_doc(
    ...
    """Finds a ChapterDoc within a BookDoc by chapter number.

    Args:
        book: The parent BookDoc object.
        chapter_number: Chapter sequence index (1-based).

    Returns:
        The matched ChapterDoc object, or None if not found.
    """
    return next(
...
def get_chapter_document(
    ...
    """Fetches a chapter document (as a dictionary) from local MongoDB.

    Args:
        translation_id: Translation abbreviation (e.g., "ESV").
        book_id: 3-letter USFM book code (e.g., "LUK").
    ...
    Returns:
        The matched chapter dictionary containing entries, or None if not found.
    """
...
def get_verse_content(entries: list[model.Entry], verse_label: str) -> str:
    """Concatenates all text fragments for a specific verse label.

    Excludes typographical metadata, footnote markers, and note tags.

    Args:
        entries: Active list of parsed Entry objects.
        verse_label: Specific verse query label (e.g., "v15").

    Returns:
        A continuous space-separated string containing the verse text.
    """
    return " ".join(
        entry.value
        ...
        and 'note' not in entry.keyParts
        and 'label' not in entry.keyParts
    )
...
def get_verse_key_parts(entries: list[model.Entry], verse_label: str) -> list:
    """Extracts CSS classes/typographic layout metadata for a verse label.

    Filters out row indices and raw 'content' selectors from keyParts.
    ...
    Returns:
        A list of typographic styles and metadata matching the verse.
    """
    for entry in entries:
...
def append_verse_plain_text(
    ...
    """Aggregates, compiles, and appends a single plain text verse Entry.

    Flushes the compiled textual content of the target verse number into
    a unified and normalized "plain_text" Entry block.

    Args:
        entries: Active list of parsed Entry objects.
        rows: Global accumulation list for CSV export logs.
        lineNumber: Current sequential row ID sequence.
        verse_qtt: Target verse number index (e.g., 15).
        file_name: Prefix metadata used to tag files. Defaults to "".

    Returns:
        The updated row sequence index (lineNumber).
    """
    if verse_qtt == 0:
        return lineNumber

    verse_label = f'v{verse_qtt}'
    verse_label_fmt = f'v{verse_qtt:03d}'
    ...
    verse_content = get_verse_content(entries, verse_label).strip()
    if len(verse_content) == 0:
        return lineNumber

    verse_text = f'{verse_qtt} ' + verse_content
    # plain_text_classes = verse_label_fmt+file_name + [lineNumber] + get_verse_key_parts(entries, verse_label) + ["plain_text"] 
    plain_text_classes = [verse_label_fmt + "_" +filename] + [lineNumber] + get_verse_key_parts(entries, verse_label) + ["plain_text"]
    if verse_label not in plain_text_classes:
        plain_text_classes.append(verse_label)
...
def get_last_plain_text_verse(entries: list[model.Entry]) -> str | None:
    """Scans parsed entries in reverse to locate the final plain-text verse tag.

    Used as an integrity verification mechanism.

    Args:
        entries: Complete list of parsed chapter Entry objects.

    Returns:
        The final parsed verse number label (e.g., "v32"), or None if not found.
    """
...
def get_book_from_json(json_file_path: str, book_id: str) -> tuple[int, str, dict] | None:
    """Resolves book details and expected verse schemas from a flat JSON manifest.

    Args:
        json_file_path: Path to the json manifest (e.g., "./app/json/old.json").
        book_id: 3-letter USFM book code (e.g., "GEN").

    Returns:
        A tuple of (book_canon_index, book_canonical_name, book_data_dict)
        if matched, or None if not found.
    """
    try:
        ...
        for chapter_num in range(1, len(book_data["chapters"]) + 1):
            ...
            if chapter_tag:
                ...
                with open(fileName_txt, "w", encoding="utf-8") as _f:
                    ...
                    entries = extract_tags_from_chapter(versionId, chapter_tag, rows, lineNumber, filename)
            ...
            entries = []  # Reset entries for the next chapter 
            rows = [["id","tag","text"]]
